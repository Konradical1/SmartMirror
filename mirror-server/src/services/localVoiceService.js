...
export async function waitForWakeWordUtterance(options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const maxCaptureMs = Number(options.maxCaptureMs || process.env.VOICE_WAKE_UTTERANCE_MAX_MS || 6500);
...
export async function speakWithElevenLabs(text, voiceId, options = {}) {
  const { play = true, signal } = options;
  const apiKey = process.env.ELEVENLABS_API_KEY;
  if (!apiKey) {
    throw new Error('ELEVENLABS_API_KEY not set in environment');
  }
...
function playAudioStream(audioStream, { label = 'TTS', signal, missingBodyMessage = 'TTS response body missing' } = {}) {
  return new Promise((resolve, reject) => {
    try {
